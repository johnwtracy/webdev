...
function listResponse(body,response) {
  ...
  var pollCount = 0;
  for (var i = 0; pollCount < 3; i++) {
    questions = polls[i].questions;
    contest = "2016 California Democratic Presidential Primary";
    for (var j=0; j<questions.length; j++) {
      if (questions[j].name == contest) {
        var HillBernUnd = [];
        pollCount++;
        HillBernUnd.push(polls[i].pollster);
        responses = questions[j].subpopulations[0].responses;
        for (var k = 0; k < responses.length; k++) {
          if (responses[k].choice == 'Clinton') 
            HillBernUnd.push(responses[k].value);
          else if (responses[k].choice == 'Sanders') 
            HillBernUnd.push(responses[k].value);
          else if (responses[k].choice == 'Undecided')
            HillBernUnd.push(responses[k].value);
        }
        while (HillBernUnd.length < 3)
          HillBernUnd.push(0);
        pollInfo.push(HillBernUnd);
      }
    }
  }
}
  
var requestString = "http://elections.huffingtonpost.com/pollster/api/polls.json?page=1&state=CA&after=2016-04-20";
...
function queryServer(request,response,search) {
    ...
    if (search == "?pollsBro")
        sendJSONData(pollInfo, response);
