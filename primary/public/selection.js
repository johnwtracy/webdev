...
makeRequest("pollsBro", fillInPolls);
...
function fillInPolls(resp) {
  
  var polls = JSON.parse(resp);
  var data = document.getElementById("pollInfo");
  var pll = document.getElementById("poll");
  ...
  for (var i = 0; i < polls.length; i++) {
    ...
    p.textContent = polls[i][0];
    pll.appendChild(p);
    p = document.createElement('p');
    p.textContent = "Hillary: " + polls[i][1] + " Bernie: " + polls[i][2] ;
    data.appendChild(p);
  }
  for (var i = 0; i < 50; i++) {
    console.log("District " + i + " Hillary: " + h + " Bernie: " + b + '\n');
  }
  
  var p = document.createElement('p');
  p.textContent = "Model Vote Prediction";
  pll.appendChild(p);
  var p2 = document.createElement('p');
  p2.textContent = "Model Delegate Prediction";
  pll.appendChild(p2);
  var p3 = document.createElement('p');
  p3.id = "percent";
  p3.textContent = "Hillary: " + percentBern
                     + " Bernie: " + percentHill;
  data.appendChild(p3);
  var p4 = document.createElement('p');
  p4.id = "delegates";
  p4.textContent = "Hillary: " + percentBern
                     + " Bernie: " + percentHill;
  data.appendChild(p4);
}
